...
const append = (message, position) => {
    ...
    // Split the message into the name and the actual message
    const messageParts = message.split(":");
    
    const nameElement = document.createElement('span');
    nameElement.innerText = messageParts[0] + ": ";  // Name part
    nameElement.style.fontWeight = "bold"; // Make the name bold
    
    const messageText = document.createElement('span');
    messageText.innerText = messageParts[1];  // Message part
...
socket.on('user-joined', (nameofperson) => {
    ...
    console.log(`${nameofperson} joined the chat`);
});
